...
if [ "$OMR_OPENWRT" = "default" ]; then
	if [ "$OMR_KERNEL" = "5.4" ]; then
		...
		_get_repo "$OMR_TARGET/source" https://github.com/suyuan168/glopenwrt6018 "test"
...
if [ "$OMR_KERNEL" != "5.15" ] && [ "$OMR_TARGET" != "x86_64" ] && [ "$OMR_TARGET" != "x86" ]; then
...
if [ "$OMR_TARGET" = "bpi-r1" ]; then
	...
	if [ "$OMR_KERNEL" != "5.15" ]; then
		echo "Sorry, but for now kernel 5.15 is the only supported one."
		exit 1
	fi
